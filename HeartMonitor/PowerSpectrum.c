...
HRVData getHeartData(){
    
    return hrvValues;
}

/*---------------------- Testing communication with the Main App Delegate ---------------------------*/
HRVData analyseData(float timeStampreceived, NSInteger heartBeatreceived, int *calmingLevel)
 {
...
void PowerSpectrum(double hrCurrent, double timeCurrent)
{
    ...
    if(numSamples > (NFFT/SAMPLING_FREQ))
    {
        ...
        if(numIntervals > NFFT)
        {
            if(firstTime == 0)
            {
                CalcStartingHeartRate(interval);
                firstTime = 1;
            }
            ...
            hrvValues.attentionLevel = CalcAttentionLevel(hrvValues.averagePower); // <----- between 1-6 (1 bad, 6 good)
            
            hrvValues.calmingLevel = CalcCalmingLevel(hrvValues.peakPowerRatio);  // <----- between 1-6 (1 bad, 6 good)
            
            hrvValues.calmAttentionLevel = CalcCalmAttentionLevel(hrvValues.attentionLevel, hrvValues.calmingLevel);  // <----- between 1-36 (1 bad, 36 perfect)
...
/* --------------------------------- Calculate Starting Heart Rate ---------------------------------------*/


void CalcStartingHeartRate(double *interval)
{
    int i;
    
    hrvValues.startingHeartRate = 0.0;
    
    for(i = 0; i < NFFT; i++)
    {
        hrvValues.startingHeartRate = hrvValues.startingHeartRate + ((double)60000.0/interval[i]);
    }
    
    hrvValues.startingHeartRate = hrvValues.startingHeartRate / (double)NFFT;
    
    if(hrvValues.startingHeartRate < 50.00)
    {
        hrvValues.startingHeartRate = 50.00;
    }
}
...
double ApplyHanningWindow(double *interval, double (*windowedData)[2])
{
    ...
    for(i = 0; i < NFFT; i++)
    {
        windowValue = (double)0.5 - ((double)0.5 * cos(  (  (double)TWO_PI * i)   /(NFFT-1))  );
...
/* ------------------------ Calculate the attention level from range 1 - 6  ---------------------------------*/
...
int CalcAttentionLevel(double *averagePower)
{
    ...
    if(averagePower[0] > 250)
    {
        attentionLevel = 1;
    }
    else if (averagePower[0] > 200)
    {
        attentionLevel = 2;
    }
...
/* ------------------------ Calculate the calming level from range 1 - 6  ---------------------------------*/
...
int CalcCalmingLevel(double peakPowerRatio)
{
    ...
    if(hrvValues.startingHeartRate > 50.00 && hrvValues.heartRate > 50.00)
    {
        multiplier = (hrvValues.startingHeartRate - MIN_HR) / (hrvValues.heartRate - MIN_HR);
    }
    peakPowerRatio = peakPowerRatio * multiplier;
    
    printf("Ratio = %.3lf, Multiplier = %.3lf\n",peakPowerRatio, multiplier);
    
    calmingLevel = 0;
    
    if(peakPowerRatio < 0.25)
    {
        calmingLevel = 1;
    }
    else if (peakPowerRatio < 0.50)
    {
        calmingLevel = 2;
    }
    else if (peakPowerRatio < 0.75)
    {
        calmingLevel = 3;
    }
...
/* ------------------------ Calculate the calm attention level from range 1 - 36  ---------------------------------*/


int CalcCalmAttentionLevel(int attentionLevel, int calmingLevel)
{
    int calmAttentionLevel;

    calmAttentionLevel = attentionLevel * calmingLevel;
    
    return calmAttentionLevel;
}
